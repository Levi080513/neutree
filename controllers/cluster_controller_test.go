...
func TestClusterController_Sync_relateImageRegistry(t *testing.T) {
	...
	testPendingImageRegistry := v1.ImageRegistry{
		ID: 1,
		Metadata: &v1.Metadata{
			Name: "test",
		},
	...
	testFailedmageRegistry := v1.ImageRegistry{
		ID: 1,
		Metadata: &v1.Metadata{
			Name: "test",
		},
	...
	tests := []struct {
		...
		{
			name: "relate image registry pending",
			input: &v1.Cluster{
				ID: 1,
				Metadata: &v1.Metadata{
					Name: "test",
			...
			mockSetup: func(s *storagemocks.MockStorage, o *orchestratormocks.MockOrchestrator) {
				s.On("ListImageRegistry", mock.Anything).Return([]v1.ImageRegistry{testPendingImageRegistry}, nil)
			},
			wantErr: true,
		},
		{
			name: "relate image registry failed",
			input: &v1.Cluster{
				ID: 1,
				Metadata: &v1.Metadata{
					Name: "test",
			...
			mockSetup: func(s *storagemocks.MockStorage, o *orchestratormocks.MockOrchestrator) {
				s.On("ListImageRegistry", mock.Anything).Return([]v1.ImageRegistry{testFailedmageRegistry}, nil)
			},
			wantErr: true,
		},
		{
		...
		{
			name: "relete image registry connected",
			input: &v1.Cluster{
				ID: 1,
				Metadata: &v1.Metadata{
					Name: "test",
...
func TestClusterController_Sync_Running(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "Running -> Running (reconcile node success)",
			input: getTestCluster(),
			mockSetup: func(input *v1.Cluster, s *storagemocks.MockStorage, o *orchestratormocks.MockOrchestrator) {
		...
		{
			name:  "Running -> Running (health check success)",
			input: getTestCluster(),
			mockSetup: func(input *v1.Cluster, s *storagemocks.MockStorage, o *orchestratormocks.MockOrchestrator) {
				o.On("GetDesireStaticWorkersIP").Return(nil)
				o.On("HealthCheck").Return(nil)
...
func TestClusterContorller_UpdateStatus(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "get ray cluster status success",
			cluster:    getTestClusterWithInitialized(),
			inputPhase: testPhase,
			inputErr:   testError,
			setupMock: func(input *v1.Cluster, m *storagemocks.MockStorage, o *orchestratormocks.MockOrchestrator) {
